...
import type { AbzaTableRowId } from '@widgets/abza-table'
...
export function AttributesTable({ onNotify }: AttributesTableProps) {
  ...
  const columns = useMemo(
    ...
    () => [

      {
        ...
        render: (row: AttributeDto) => row.name,
...

      {
        ...
        render: (row: AttributeDto) => row.description ?? '—',
...

      {
        ...
        render: (row: AttributeDto) => t(`attributes.valueTypes.${row.valueType}`, row.valueType),
...

      {
        ...
        render: (row: AttributeDto) => t(`attributes.inputTypes.${row.inputType}`, row.inputType),
  ...
  const handleLinkSelected = async () => {
    ...
    try {
      ...
      await linkAttributesToProfileBatch(
